...
import { ChatGameCreator } from '@/modules/coinleague/components/ChatGameCreator';
...
const CoinLeagueIndex: NextPage = () => {
  ...
  const [createGameInitialValues, setCreateGameInitialValues] = useState<Partial<{
    gameType: number;
    duration: number;
    gameLevel: number;
    maxCoins: number;
  ...
  const handleCloseCreateGame = () => {
    ...
    setCreateGameInitialValues(undefined);
  };
  ...
  const handleGameParamsExtracted = (params: {
    gameType?: number;
    duration?: number;
    gameLevel?: number;
    maxCoins?: number;
    ...
    setCreateGameInitialValues(params);
    setShowCreateGame(true);
  };
  ...
  return (
    <>
      ...
      <MainLayout>
        <Stack spacing={2}>
          <ChatGameCreator onGameParamsExtracted={handleGameParamsExtracted} />

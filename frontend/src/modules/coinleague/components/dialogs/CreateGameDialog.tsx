...
export default function CreateGameDialog({ dialogProps, initialValues }: Props) {
  ...
  const form = useFormik<Form>({
    ...
    initialValues: {
      gameLevel: initialValues?.gameLevel ?? GameLevel.Novice,
      gameType: initialValues?.gameType ?? GameType.Bull,
      maxCoins: initialValues?.maxCoins ?? 2,
      duration: initialValues?.duration ?? (process.env.NODE_ENV === 'development' ? 60 * 60 : 60 * 60),
      maxPlayers: initialValues?.maxPlayers ?? 50,
      startDate: initialValues?.startDate ?? moment().toDate().getTime(),
    },
  ...
  useEffect(() => {
    if (initialValues) {
      if (initialValues.gameLevel !== undefined) {
        form.setFieldValue('gameLevel', initialValues.gameLevel);
      }
      if (initialValues.gameType !== undefined) {
        form.setFieldValue('gameType', initialValues.gameType);
      }
      if (initialValues.maxCoins !== undefined) {
        form.setFieldValue('maxCoins', initialValues.maxCoins);
      }
      if (initialValues.duration !== undefined) {
        form.setFieldValue('duration', initialValues.duration);
      }
      if (initialValues.maxPlayers !== undefined) {
        form.setFieldValue('maxPlayers', initialValues.maxPlayers);
      }
            ...
            <FormControl>
              ...
              >
                {process.env.NODE_ENV === 'development' && (
                  <MenuItem value={60 * 5}>
                    <FormattedMessage id="5minutes" defaultMessage="5 minutes" />
                  </MenuItem>
                )}
